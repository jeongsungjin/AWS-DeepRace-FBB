...
def closest_optimal_waypoint(agent_x, agent_y):
    distances = []
    ...
    return distances[0][0]  # 가장 가까운 웨이포인트 반환
...
def reward_function(params):
    ...
    # 최적 웨이포인트 찾기
    closest_wp = closest_optimal_waypoint(agent_x, agent_y)
    wp_x, wp_y, optimal_speed = closest_wp

    # 최적 속도 보상
    if optimal_speed * 0.92 <= agent_speed <= optimal_speed * 1.08:
        reward += 1.0  # 속도가 최적 속도 이상일 때 보상

    # 속도와 조향 상호작용
    if agent_speed >= 2.0:
        steering_penalty = max(0.001, 1 - (abs(steering_angle) / 10))  #차이가 생길수록 리워드 감소
        reward *= steering_penalty

    # 웨이포인트 방향과 차량 헤딩 비교
    wp_heading = math.degrees(math.atan2(wp_y - agent_y, wp_x - agent_x))
    heading_diff = abs(params['heading'] - wp_heading)
    heading_diff = min(heading_diff, 360 - heading_diff)  # 0~180도로 제한
    heading_reward = max(0.001, 1 - (heading_diff / 10))  
