...
def closeset_two_optimal_waypoint(agent_x, agent_y): # 에이전트와 가장 가까운 최적화 waypoint 두개 반환
    distances = []
    # waypoint들 
...
def distance_from_optimal_path(agent_x, agent_y): # 가장 가까운 waypoint 두개를 이은 직선과 에이전트와의 거리 반환
    waypoint1, waypoint2 = closeset_two_optimal_waypoint(agent_x, agent_y)
...
def get_reward_acrobatic(all_wheels_on_track, is_offtrack, closeset_wps): # 코너링을 할때 아슬아슬하게 돌면 큰 보상 리턴 (보상범위 : 1.0 ~ 1.5)
    corners = [[16,21],[28,32],[46,52],[60,66],[73,78],[80,84],[88,93]] # [코너시작 waypoint, 코너끝 waypoint]로 이루어진 리스트
    is_corner = False
    reward = 0.0

    for corner in corners: # 지금 agent가 코너에 있는가 판별하는 코드
        if closeset_wps[0] in list(range(corner[0],corner[1])) or closeset_wps[1] in list(range(corner[0],corner[1])):
            is_corner = True

    if not is_offtrack and not all_wheels_on_track and is_corner: # 코너링할때 바퀴가 트랙에서 벗어났는데 아예 벗어나진 않으면 보상
        reward = 1.5
    else:
        reward = 1.0
...
def reward_function(params):
    ...
    closeset_wps = params['closeset_waypoints']

    # 상황에 따른 보상 초기화
    reward_path = get_reward_path(agent_x, agent_y)
    reward_acrobatic = get_reward_acrobatic(all_wheels_on_track, is_offtrack, closeset_wps)

    # 가중치 부여해서 최종 보상 초기화
    reward = 1*reward_path * reward_acrobatic
